...
describe('damage', function () {
    ...
    test('hp should drop from 32 to 22 when applying fire damage effect', function () {
        const c = oManager.createCreature({ id: 'm1', ref: 'c-centaur' })
        expect(c.getters.getHitPoints).toBe(32)
        const eDamage = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE, 10, { type: CONSTS.DAMAGE_TYPE_FIRE })
    ...
    test('hp should drop from 32 to 27 when applying fire damage effect and fire resistance effect', function () {
        const c = oManager.createCreature({ id: 'm1', ref: 'c-centaur' })
        expect(c.getters.getHitPoints).toBe(32)
        const eResist = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE_RESISTANCE, 0, { type: CONSTS.DAMAGE_TYPE_FIRE })
        const eDamage = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE, 10, { type: CONSTS.DAMAGE_TYPE_FIRE })
    ...
    test('hp should no drop from 32 to 27 when applying fire damage effect and fire immunity effect', function () {
        const c = oManager.createCreature({ id: 'm1', ref: 'c-centaur' })
        expect(c.getters.getHitPoints).toBe(32)
        const eImm = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE_IMMUNITY, 0, { type: CONSTS.DAMAGE_TYPE_FIRE })
        const eDamage = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE, 10, { type: CONSTS.DAMAGE_TYPE_FIRE })
    ...
    test('hp should no drop from 32 to 12 when applying fire damage effect and fire vulnerability effect', function () {
        const c = oManager.createCreature({ id: 'm1', ref: 'c-centaur' })
        expect(c.getters.getHitPoints).toBe(32)
        const eVul = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE_VULNERABILITY, 0, { type: CONSTS.DAMAGE_TYPE_FIRE })
        const eDamage = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE, 10, { type: CONSTS.DAMAGE_TYPE_FIRE })
    ...
    it('should be immune to weapon damage but non-immune to silver weapons', function () {
        const c = oManager.createCreature({ id: 'm1', ref: 'c-centaur' })
        expect(c.getters.getHitPoints).toBe(32)
        const eVulSilv = oManager.effectProcessor.createEffect(CONSTS.EFFECT_MATERIAL_VULNERABILITY, 0, { type: CONSTS.MATERIAL_SILVER })
        const eImmWeapon = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE_IMMUNITY, 0, { type: CONSTS.DAMAGE_TYPE_PHYSICAL })
        const eSteelDamage = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE, 10, { type: CONSTS.DAMAGE_TYPE_PHYSICAL, material: CONSTS.MATERIAL_STEEL })
        const eSilverDamage = oManager.effectProcessor.createEffect(CONSTS.EFFECT_DAMAGE, 10, { type: CONSTS.DAMAGE_TYPE_PHYSICAL, material: CONSTS.MATERIAL_SILVER })
